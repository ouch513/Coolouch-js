...
;(function($){
    ...
    var DATA_NAME = 'jsb_data',
        BAR_WRAPPER_NODE = $('<div class="COScroll" style="overflow: hidden"></div>'),
    ...
    function init(width, height, barWidth){
        this.$node.css('overflow','hidden').addClass('COScroll')
    ...
    function testXYShow(width, height, barWidth){
        var //node = this.node,
            ...
            opt = this.opt,
            con = this.$con[0],
            $cloneNode = this.$node.clone().appendTo('body').width(width).height(height).find('.COScroll'),
            //$cloneNode = $cloneNode.find('.COScroll'),
     ...
     */
    COScrollBar.fn.getThumbLocation = function(dir){
     ...
     * @returns {*}
     */
    COScrollBar.fn.getScrollPos = function(dir){
     ...
     */
    COScrollBar.fn.scroll = function(direction){
     ...
     */
    COScrollBar.fn.scrollBy = function(dir, amount){
     ...
     * @param {Number} speed
     */
    COScrollBar.fn.scrollTo = function(direction, target, speed){
    ...
    $.fn.COScroll = function(opt){
        ...
        return this.each(function(){
            ...
            if(!objData){
                $.data(this, DATA_NAME, new COScrollBar($(this), opt))
            }
        })
    };
})(jQuery);
...
(function ($, window, document, undefined) {
    ...
    function COMODAL(ele, opt) {
        ...
        that.$comodalOverlay = $('<div class="comodalOverlay ' + that.overlayZIndex + '"></div>');
    ...
    COMODAL.prototype = {
        tpl: {
            comodal: [
                '<div>',
                '<div class="comodal_header">',
                '<div class="comodal_title"></div>',
                '</div>',
                '<div class="comodal_body">',
                '</div>',
                '<div class="comodal_footer">',
                '</div>',
                '</div>'
            ].join(''),
            loading: '<div class="comodalLoadingText"></div>',
            closeNormal: '<a class="comodal_header_close_normal"></a>',
            closeCircle: '<a class="comodal_header_close_circle"></a>',
            iframe: '<iframe class="comodal_iframe" frameborder="0" vspace="0" hspace="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen' + (IE ? ' allowtransparency="true"' : '') + ' width="100%" height="100%"></iframe>'
        },

        init: function () {
            //console.log('COModal init');
            var that = this;
        ...
        createContainer: function () {
            var that = this,
                ...
                $comodalTpl = $(that.tpl.comodal),
                $header = $comodalTpl.find('.comodal_header'),
                $title = $comodalTpl.find('.comodal_title'),
                $close = that.options.closeType.toLowerCase() === 'in' ? $(that.tpl.closeNormal) : $(that.tpl.closeCircle),
                $body = $comodalTpl.find('.comodal_body'),
                $footer = $comodalTpl.find('.comodal_footer'),
            ...
            if (that.options.isShowTitle) {
                ...
                }).addClass('comodal_header_border');
        ...
        setValue: function () {
            var that = this;
            that.$header = that.$element.find(".comodal_header");
            that.$title = that.$element.find(".comodal_title");
            that.$close = that.options.closeType === 'in' ? that.$element.find('.comodal_header_close_normal') : that.$element.find('.comodal_header_close_circle');
            that.$body = that.$element.find(".comodal_body");
            that.$footer = that.$element.find(".comodal_footer");
        },
        ...
        asyncLoadImage: function () {
            ...
            img.onload = function () {
                $(img).addClass('comodal_body_img_center');
                that.$body.html(img).addClass('comodal_img_center');
                that.loadSuccess();
                that.afterLoad();
            }
        ...
        beforeLoad: function () {
            ...
            if (that.ajaxOption.isShowLoading) {
                if (that.ajaxOption.loadingType === 'img') {
                    that.$body.addClass('comodalLoadingImg');
                } else {
        ...
        loadError: function () {
            ...
            that.$body.removeClass('comodalLoadSuccess');
        },

        loadSuccess: function () {
            this.$body.addClass('comodalLoadSuccess');
        },
        ...
        afterLoad: function () {
            ...
            if (that.ajaxOption.isShowLoading) {
                if (that.ajaxOption.loadingType === 'img') {
                    that.$body.removeClass('comodalLoadingImg');
                } else if (that.ajaxOption.loadingType === 'text' && that.ajaxOption.contentType.toLowerCase() === 'iframe') {
                    that.$body.find('.comodalLoadingText').remove();
                }
            }
        },

        isLoadSuccess: function () {
            return this.$body.hasClass('comodalLoadSuccess');
        },
        ...
        dragBox: function () {
            ...
            that.$title.bind('mousedown', function (M) {
                ...
                if (that.options.dragType.toLowerCase() === 'replace') {
                    ...
                    that.$drag = $('<div class="COModal_drag" style="left:' + xx + 'px; top:' + yy + 'px; width:' + ww + 'px; height:' + hh + 'px; z-index:2147483647"></div>');
                    that.$drag.css({
                        'border-radius':that.options.borderRadius,
                        '-moz-border-radius':that.options.borderRadius,
                        '-webkit-border-radius':that.options.borderRadius
        ...
        bindEvent: function () {
            ...
            if (that.options.isCloseOnOverlayClick && that.options.isShowOverlay) {
                that.$comodalOverlay.bind('click', function (e) {
                    that.close();
                });
            }
            ...
            if (that.options.isDrag) {
                that.$title.addClass('comodal_title_move');
                that.dragBox();
            }
        },
        ...
        unbindEvent: function () {
            ...
            that.$comodalOverlay.unbind('click');
    ...
    $.fn.COModal = function (options, args) {
        var dataKey = 'COModalKey';

        return this.each(function () {

            var COModalElement = $(this),
                instance = COModalElement.data(dataKey);
            ...
            if (typeof options === 'string') {
                if (instance && typeof instance[options] === 'function') {
                    ...
                    console.error('you should instance jquery.COModal before use it!');
                } else {
                    console.error('method ' + options + ' does not exist on jquery.COModal!');
                }
            } else {
                instance = new COMODAL(this, options);
                instance.init();

                COModalElement.data(dataKey, instance);
            }
        });
    }
...
/**
 * COSlide-simple 简版轮播 插件
 */
(function($) {
    $.fn.COSlideSimple = function(options) {
        var defaults = {
            direction: 'left',
            duration: 0.6,
            easing: 'swing',
...
(function($) {
    ...
    $.fn.COSlide3D = function (options) {

        options = $.extend({}, $.fn.COSlide3D.defaults, options || {});
...
$(function(){
	$(".coslide-3d").each(function(){
		var cw = parseInt($(this).width());
		if( cw >= '760' && cw < '1050'){
			$(this).find('img').width(450);
			$(this).find('img').height(230);
		}else if( cw < '480'){
			$(this).height(170);
			$(this).find('img').width(200);
			$(this).find('img').height(100);
		}else if( cw >= '480' && cw < '760'){
			$(this).height(250);
			$(this).find('img').width(350);
			$(this).find('img').height(180);
		}else if( cw >='1050' && cw < '1300'){
			$(this).height(400);
			$(this).find('img').width(600);
			$(this).find('img').height(310);
		}
		else if( cw >='1300'&& cw < '1750'){
			$(this).height(500);
			$(this).find('img').width(800);
			$(this).find('img').height(410);
		}else if( cw >='1700'){
			$(this).height(750);
			$(this).find('img').width(1200);
			$(this).find('img').height(620);
		}
	});
});
...
( function( $, window, undefined ) {
	...
	$.COSlidecircle = function( options, element ) {
		this.$el = $( element );
		this._init( options );
	};

	// the options
	$.COSlidecircle.defaults = {
	...
	$.COSlidecircle.prototype = {
		_init : function( options ) {
			
			// options
			this.options = $.extend( true, {}, $.COSlidecircle.defaults, options );
			...
			if( this.support ) {
				this.transEndEventName = transProperties[ Modernizr.prefixed( 'transition' ) ].transitionEndEvent + '.COSlideCircle';
				this.transformName = transProperties[ Modernizr.prefixed( 'transition' ) ].transformName;
			}
			this.transitionProperties = this.transformName + ' ' + this.options.speed + 'ms ' + this.options.easing;

			this.$listItems = this.$el.children( 'ul.cosc-slides' );
		...
		_addNav : function() {
			...
			var self = this,
				$navLeft = $( '<div class="cosc-left"><span></span><span></span><span></span><i class="icon-arrow-left"></i></div>' ),
				$navRight = $( '<div class="cosc-right"><span></span><span></span><span></span><i class="icon-arrow-right"></i></div>' );

			$( '<nav></nav>' ).append( $navLeft, $navRight ).appendTo( this.$el );

			$navLeft.find( 'span' ).on( 'click.COSlideCircle touchstart.COSlideCircle', function() {
				self._navigate( $( this ), 'left' );
			} );

			$navRight.find( 'span' ).on( 'click.COSlideCircle touchstart.COSlideCircle', function() {
		...
		_layout : function( $current, $next ) {
			...
			this.$flipFront = $( '<div class="cosc-front"><div></div></div>' );
			this.$frontContent = this.$flipFront.children( 'div:first' );
			this.$flipBack = $( '<div class="cosc-back"><div></div></div>' );
			this.$backContent = this.$flipBack.children( 'div:first' );
			this.$flipEl = $( '<div class="cosc-flip"></div>' ).append( this.$flipFront, this.$flipBack ).hide().appendTo( this.$el );
		...
		_flip : function( $currentItem, $nextItem, dir, angle ) {
			...
			var transformProperties = '',
				// overlays
				$overlayLight = $( '<div class="cosc-overlay-light"></div>' ),
				$overlayDark = $( '<div class="cosc-overlay-dark"></div>' );

			this.$flipEl.css( 'transition', this.transitionProperties );
			
			this.$flipFront.find( 'div.cosc-overlay-light, div.cosc-overlay-dark' ).remove();
			this.$flipBack.find( 'div.cosc-overlay-light, div.cosc-overlay-dark' ).remove();
			...
			setTimeout( function() {
				...
				self.$flipEl.on( self.transEndEventName, function( event ) {
					if( event.target.className === 'cosc-overlay-light' || event.target.className === 'cosc-overlay-dark' ) return;
	...
	$.fn.COSlideCircle = function( options ) {
		if ( typeof options === 'string' ) {
			var args = Array.prototype.slice.call( arguments, 1 );
			this.each(function() {
				var instance = $.data( this, 'COSlideCircle' );
				if ( !instance ) {
					logError( "cannot call methods on COSlideCircle prior to initialization; " +
					"attempted to call method '" + options + "'" );
					return;
				}
				if ( !$.isFunction( instance[options] ) || options.charAt(0) === "_" ) {
					logError( "no such method '" + options + "' for COSlideCircle instance" );
					return;
				}
			...
			this.each(function() {	
				...
				if ( instance ) {
					...
					instance = $.data( this, 'COSlideCircle', new $.COSlidecircle( options, this ) );
				}
			});
		}
		return this;
...
$(function(){
	$(".coslide-circle").COSlideCircle();
	$(".coslide-circle").each(function(){
		var cw = parseInt($(this).width());
		$(this).find('img').width(cw);
		$(this).find('img').height(cw);
	});
});
...
(function($) {
	...
	$.fn.COMenuDrop = function(options){
		var settings = $.extend({
			...
			'FontColor':'#777',
			'FontHoverColor':'#169FE6',
			'FontBg':'white',
			'FontHoverBg':'white',
			
			'SubFontColor':'#777',
			'SubFontHoverColor':'#000',
			'SubFontBg':'white',
			'SubFontHoverBg':'#ECF5FF',
			'SubBorder':'#ECF5FF',
		...
		$(this).height(settings.MainMenuHeight);
		$(this).find('li').height(settings.MainMenuHeight);
		$(this).find('li a').height(settings.MainMenuHeight);
		$(this).find('li a').css('line-height', function(){ return settings.MainMenuHeight+"px";});
		$(this).find('.sub-nav').css('top', function(){ return settings.MainMenuHeight+"px";});
			
		$(this).find('.sub-nav li').height( settings.SubMenuHeight);
		$(this).find('.sub-nav li a').height( settings.SubMenuHeight);
		$(this).find('.sub-nav li a').css('line-height', settings.SubMenuHeight+'px');
		$(this).find('.sub-nav ul').css('top','7px');
	
		$(this).find('.sub-nav').width( settings.SubMenuWidth);
		$(this).find('.sub-nav li').width( settings.SubMenuWidth);
		$(this).find('.sub-nav li a').width( settings.SubMenuWidth);
		$(this).find('.sub-nav ul').css('left', settings.SubMenuWidth+"px");
		
		if($(this).hasClass('comenu-vertical')){
			$(this).find('li').width(settings.SubMenuWidth);
			$(this).find('.sub-nav').css('left', settings.SubMenuWidth+"px");
			$(this).find('.sub-nav').css('top', '7px');				
		}
		
		$(this).find('a').css('color', settings.FontColor);
		$(this).find('a').css('background-color', settings.FontBg);
		$(this).find('a').hover(function() {
			$(this).css('color', settings.FontHoverColor);
			$(this).css('background-color', settings.FontHoverBg);
		}, function() {
			$(this).css('color', settings.FontColor);
			$(this).css('background-color', settings.FontBg);					
		});
		
		$(this).find('.sub-nav a').css('color', settings.SubFontColor);
		$(this).find('.sub-nav').css('background-color', settings.SubFontBg);
		$(this).find('.sub-nav ul').css('background-color', settings.SubFontBg);
		$(this).find('.sub-nav a').css('background-color', settings.SubFontBg);
		$(this).find('.sub-nav li').css('border-color', settings.SubBorder);
		$(this).find('.sub-nav a').hover(function() {
			$(this).css('color', settings.SubFontHoverColor);
			$(this).css('background-color', settings.SubFontHoverBg);
		}, function() {
			$(this).css('color', settings.SubFontColor);
			$(this).css('background-color', settings.SubFontBg);
		});
		
		$(this).find('.active a:first').css('color', settings.ActiveColor);
		$(this).find('.active a:first').css('background-color', settings.ActiveBg);
		$(this).find('.active a:first').hover(function() {
			$(this).css('color', settings.ActiveColor);
			$(this).css('background-color', settings.ActiveBg);
		}, function() {
			$(this).css('color', settings.ActiveColor);
			$(this).css('background-color', settings.ActiveBg);					
		});
		...
		if(($(this).find('.mobile-menu').length>0)&&(!$(this).hasClass('comenu-vertical'))){
			
			$(this).find('.mobile-menu').css('margin', function(){ return (settings.MainMenuHeight-35)/2+"px";});
			$(this).append('<div class="mobile-menu-list"></div>');
			$(this).find('.mobile-menu-list').css('top', function(){ return settings.MainMenuHeight+"px";});
			$(this).find('.mobile-menu-list').css('right', function(){ return (settings.MainMenuHeight-35)/2+"px";});
			$(this).find('.mobile-menu-list').append('<ul class="mobile-menu-ul"></ul>');
			var certainLi = $(this).find('li');
			$(this).find('.mobile-menu-list .mobile-menu-ul').append(function(){
				var lists="";
				certainLi.each(function(){
					lists += '<li class="item">';
					var pl = $(this).parents('li').length;
					if(pl>0){
						for(var i=0;i<pl;i++){
							lists +='&emsp;';
						}
			...
			$(this).find('.mobile-menu-list .mobile-menu-ul li').unbind();
			
			if($(window).width()<750){
				$(this).find('li').not('.mobile-menu-list li').css('display','none');
			...
			$(window).resize(function() {	
				thisUl.find('.mobile-menu-list').hide();
				if($(window).width()<750){
					thisUl.find('li').not('.mobile-menu-list li').css('display','none');
					thisUl.find('.mobile-menu').show();
					thisUl.css('float','right');
				}else{
					thisUl.find('li').css('display','block');
					thisUl.find('.mobile-menu').hide();
					thisUl.css('float','left');
				}
			});
			
			
			$(this).find('.mobile-menu').click(function(){
				$(this).parents('.comenu-drop').find('.mobile-menu-list').toggle('slow');
			});
				
			$(this).find('.mobile-menu-list .mobile-menu-ul').css('position', 'static');
			$(this).find('.mobile-menu-list .mobile-menu-ul').css('width', '100%');
			$(this).find('.mobile-menu-list .mobile-menu-ul').css('height', 'initial');
			$(this).find('.mobile-menu-list .mobile-menu-ul').css('background-color', '#f0f0f0');
			$(this).find('.mobile-menu-list .mobile-menu-ul').css('padding', '0');
			$(this).find('.mobile-menu-list .mobile-menu-ul').css('margin', 0);
			$(this).find('.mobile-menu-list .item').css('position', 'static');
			$(this).find('.mobile-menu-list .item').css('width', '100%');			
			$(this).find('.mobile-menu-list .item ').css('height', '30px');
			$(this).find('.mobile-menu-list .item').css('background-color', '#f0f0f0');
			$(this).find('.mobile-menu-list .item').css('padding', 0);
			$(this).find('.mobile-menu-list .item').css('margin', 0);
			$(this).find('.mobile-menu-list .item a').css('background-color', '#f0f0f0');
			$(this).find('.mobile-menu-list .item a').css('padding', '0 10px');
			$(this).find('.mobile-menu-list .item a').css('margin', 0);
			$(this).find('.mobile-menu-list .item a').css('height', '29px');
			$(this).find('.mobile-menu-list .item a').css('line-height', '29px');
			$(this).find('.mobile-menu-list .item a').css('color', '#3c3c3c');
			$(this).find('.mobile-menu-list .item a').css('display', 'inline-block');
...
(function($){
	$.fn.COMenuArc=function(options){
		...
		var settings = $.extend({	
			...
			'SubFontColor':'white',
			'SubFontBg':'#4ECDC4',
			'SubFontHoverColor':'white',
			'SubFontHoverBg':'#4ECDC4'	
		}, options);
		
		$(this).css('width',settings.size+'px');
		$(this).css('height',settings.size+'px');
		$(this).find('.comenu-arc-main').css('width',settings.size+'px');
		$(this).find('.comenu-arc-main').css('height',settings.size+'px');
		$(this).find('.comenu-arc-main').css('line-height',settings.size+'px');
		$(this).find('.comenu-arc-list').css('width',settings.size+'px');
		$(this).find('.comenu-arc-list').css('height',settings.size+'px');		
		$(this).find('.comenu-arc-list').css('line-height',settings.size+'px');
		$(this).find('a').css('width',settings.size+'px');
		$(this).find('a').css('height',settings.size+'px');
		
		$(this).find('.comenu-arc-main').css('color', settings.FontColor);
		$(this).find('.comenu-arc-main').css('background-color', settings.FontBg);
		$(this).find('.comenu-arc-main').hover(function() {
			$(this).css('color', settings.FontHoverColor);
			$(this).css('background-color', settings.FontHoverBg);
		}, function() {
			$(this).css('color', settings.FontColor);
		...
		$(this).find('a').css('color', settings.SubFontColor);
		$(this).find('a').css('background-color', settings.SubFontBg);
		$(this).find('li').hover(function() {
			$(this).find('a').css('color', settings.SubFontHoverColor);
			$(this).find('a').css('background-color', settings.SubFontHoverBg);
		}, function() {
			$(this).find('a').css('color', settings.SubFontColor);
			$(this).find('a').css('background-color', settings.SubFontBg);					
		});
		...
		if(!((settings.direction=='bl')||(settings.direction=='br')||(settings.direction=='tl')||(settings.direction=='tr'))){
			settings.direction=='bl';
		}
		
		var ul=$(this).find('.comenu-arc-list'),li=$(this).find("li"),i=li.length,n=i-1,r=settings.expandRadius;
		if(settings.direction=='bl'){
			$(this).css('top','initial');
			$(this).css('right','initial');
			$(this).css('bottom',settings.margin+'px');
			$(this).css('left',settings.margin+'px');
			
			$(this).find('.comenu-arc-main').click(function(){
		...
		if(settings.direction=='br'){
			$(this).css('top','initial');
			$(this).css('left','initial');
			$(this).css('right',settings.margin+'px');
			$(this).css('bottom',settings.margin+'px');
		...
		if(settings.direction=='tl'){
			$(this).css('bottom','initial');
			$(this).css('right','initial');
			$(this).css('top',settings.margin+'px');
			$(this).css('left',settings.margin+'px');			
...
(function($) {
	...
	$.extend($.fn, {
		...
		applyClasses: function(settings, toggler) {
			this.filter(":has(>ul):not(:has(>a))").find(">span").unbind("click.COMenuTree").bind("click.COMenuTree", function(event) {
	...
	$.COMenuTree = {};
	var CLASSES = ($.COMenuTree.classes = {
		open: "open",
		closed: "closed",
...
(function ($, window) {
    ...
    var effect = {
        ...
        seTemplet: function () {
            ...
            for (var i = 0; i < config.level; i++) { //循环定义的级别
                ...
                if (config.renderMode) {
                    //模拟
                    selectemplet += '<div class="item storey ' + citygrade + '" data-index="' + i + '">'
                        +'<a href="javascript:;" class="selector reveal df-color ' + forbid + '">' + placeholder + '</a>'
    ...
    $.fn.COSelectCity = function (options) {
 ...
 */
$(function() {
	$('.coform .checkbox-item').bind('click',function(){
	...
	$('.coform .radio-item').bind('click',function(){
...
/*
 *Coolouch Accordion 手风琴
 */
(function($) {
	$.fn.COAccordion = function(){
		$(this).find('.title:first').addClass('active');
		$(this).find('.title:first').next(".text").addClass('active-div');
		$(this).find('.title').click(function(){
			$(this).siblings().removeClass('active');
			$(this).addClass('active');
			$(this).siblings('.title').each(function(){ $(this).next(".text").hide('normal'); });;
			$(this).next(".text").show('slow');
		});
	};
})(jQuery);
$(function(){
	$('.cotext-accordion').COAccordion();
});

/*
 *Coolouch Jump Text 动态文字
 */
(function($) {
    $.fn.COTextJump = function(options) {
        var defaults = {
            JumpHeight: '1em',
            upTime: 80,
            downTime: 80,
        ...
        return this.each(function() {
            ...
            for (var i = 0; i <= text.length; i++) {
                ...
                newMarkup += ($.trim(character)) ? '<span class="beat-char" style="font-family:'+options.fontFamily+';font-size:'+options.fontSize+';font-weight:'+options.fontWeight+';color:'+options.fontColor+';">' + character + '</span>' : character
            }
            obj.html(newMarkup);
			if(!options.isAuth){			
...
$(function(){
	$(".cotext-hover").each(function(){$(this).hoverdir()});
});

/**
 * COSelectTime 时间选择 插件
 */
(function(root,factory,plug){
    ...
    var __prop__ = {
        _init : function(){
            ...
            var less = this.less;
            COSelectTimeEvent(start,end,level,less);
        }
    };
...
function onLoadTimeChoiceDemo(){
    var demo = '<div class="borain-timeChoice-body">'+
        ...
        '<div class="choiceBox">'+
        '<div class="yearBox">'+
        '<a class="btn back" href="javascript:void(0);">&lsaquo;</a>'+
        '<div class="showBox">'+
        '<a class="year" href="javascript:void(0);"></a> - <a class="month" href="javascript:void(0);"></a>'+
        '</div><a class="btn forward" href="javascript:void(0);">&rsaquo;</a></div>'+
        ...
        '<a class="btn up fl" href="javascript:void(0);">&lsaquo;</a>'+
        '<span class="fl"><em></em> - <em></em></span>'+
        '<a class="btn down fr" href="javascript:void(0);">&rsaquo;</a>'+
        '</div>'+
        '<div class="year">'+
        '<!--加载年-->'+
        '</div>'+
        ...
        '<div class="timeBox">'+
        '<div class="hourBox">'+
        '<a class="add" href="javascript:void (0);">+</i></a>'+
        '<span></span>'+
...
//时间选择器初始化
function COSelectTimeEvent(start,end,level,less){
...
//关闭时间选择控件
function COSelectTimeCancel(start,end){
    var mask = $(".borain-timeChoiceMask");
    var box = $(".borain-timeChoice");
...
/**
 * COModalGalery 弹层览图 插件
 */
;(function($){
	$.fn.COModalGalery = function(){
		var $obj = this;
